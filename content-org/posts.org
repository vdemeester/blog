...
** knative                                       :kubernetes:knative:docker:

** minikube tricks                              :minikube:kubernetes:docker:

*** TODO Getting started with minikube        :cri:o:cri:containerd:docker:

**** TODO Start a minikube

***** TODO Alternate runtimes

**** TODO Cache some images
...
/2018[fn:2] reviews/[fn:3] articles[fn:4] in my feedly feed and I thought it would be a good idea to write my
own too.
...
Before that, at Docker Inc., I continued the work I started the year before,
a.k.a. maintaining the Moby project and the docker engine, amongst other Docker project
...
At Red Hat, I started to work upstream in the Kubernetes community, mainly on the Knative porjects.
I also work on the Openshift Cloud Function project (and thus team), and those fellows are
awesome ! Digging more into Openshift, and other part of the Red Hat portfolio is a really
good learning experience, and it's just the start !

As stated above, I am now working home, full-time. I could work from home when I was at
Docker inc, but wokring home full-time is another kind of beast. So far it is really good,
some adjustments were needed but it's for the best. Here is a small take on "working from
home":
...
- It's also really easy to work long day or really long period of time. It's especially
  ...
  - I ended up using the Pomodoro technique to make sure I move few times a day
...
- It's easy to adapt your day to circompstence.L If you got to run errands in the middle
  of the day, it's no big deal. You can take the time back later on.
...
way less. End of august, I felt something weird on the right knee, and well, turns out my
...
gonna be able to move around much but.. meh).
...
few months after the surgery 😅). I also invested on a ultrawide screen, to get the same
...
This year I also migrate any of my "infrastructure" computer to NixOS. I learned a lot of
Nix this year, reworked my configuration multiple time to end up with a [[https://github.com/vdemeester/nixos-configuration.git][system
configuration repository]] that uses modules, and a [[https://github.com/vdemeester/home.git][/home rconfiguration repository/]] (for
user configuration). The [[https://github.com/vdemeester/home.git][home]] repository uses [[https://github.com/rycee/home-manager.git][=home-manager=]] and thus doesn't make any
assumption of running on top of NixOS. This allows me to have an easy to get setup on any
system that Nixpkgs support (any Linux distribution, Mac OSX, Windows Subsystem
Linux). The current configuration is not yet optimal but I'm pretty happy about what I
got :
...
- Local proxies and mirrors for docker images, nixpkgs binary package and go modules to
  eat less bandwidth.
- Easy to setup VPN using wiregurad.
...
to also use =org-mode= for task management. I'm in the /transition/ mode right now, but my
...
I like to read, but the past years, I didn't really read too much, except some technical
books. 2018 in the respect is not an exception, I didn't read too much. Worse than that, I
started some book and stopped at some point, for no apparent reason ; and now, I need to
start back from the beginning, which, well, is not helping me want to read them again.
...
** TODO i3 tips & tricks                                        :desktop:i3:
