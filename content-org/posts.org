...
** Nix                                                               :nixos:
*** DRAFT =nix run= aliases
:PROPERTIES:
:EXPORT_FILE_NAME: 2019-01-26-nix-run-alias.md
...
The =nr= tool would have to be able to manage multiple /profile/, which really stands
...
Finally, avoiding conflicts at /build time/ (=home-manager switch=) is a bit tricky. One way to
achieve it is to use =file.?.onChange= script, which is executed after [[https://github.com/rycee/home-manager][=home-manager=]] has
