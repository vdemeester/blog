...
*** DRAFT Golang testing — gotest.tools icmd                 :exec:command:
:PROPERTIES:
...
After filesystem operation (seen in [[/posts/2018-09-14-gotest-tools-fs/][=fs=]]), one pretty common use-case in tests is to
...
would be really easy to /shadow/ and have some really weird problems going on.
...
Finally, we listed =Equal= above, that returns a =Comparison= struct. This means we can use
it easily with the =assert= package. As written in a [[/posts/2018-08-16-gotest-tools-assertions/][previous post (about the =assert=
package)]], I prefer to use =cmp.Comparison=. Let's convert the above examples using =assert=.
